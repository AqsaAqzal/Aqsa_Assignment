...
public class InventoryResourceTest {
    ...
    public void insertIntoTable() {
        ...
        try {
            assertEquals(Response.ok().build(), ir.addNewInventoryItem(request, payload));
        }
        catch(Exception ex) {
            ex.getStackTrace();
        }
    ...
    public void updateTable() {
        ...
        try {
            assertEquals(Response.ok().build(), ir.updateExistingInventoryItem(request, payload));
        }
        catch(Exception ex) {
            ex.getStackTrace();
        }
  ...
  @Test
    public void deleteFromTable() throws SQLException {
      ...
      ContainerRequestContext request = null;
      int id = 3;
        Response res = ir.deleteExistingInventoryItem(request, id);
        try {
...
-
        try {
            assertEquals(str, ir.fetchInventoryItemById(request, id));
        }
        catch(Exception ex) {
            ex.getStackTrace();
        }
    ...
    public void allItemsOfTable() {
        ...
        try {
            assertEquals(str, ir.fetchAllInventoryItems(request));
        }
        catch(Exception ex) {
            ex.getStackTrace();
        }
    ...
    public void itemWithCategoryIdequalsTo2() {
        ...
        try {
            assertEquals(str, ir.fetchAllInventoryItemsByCategory(request, category));
        }
        catch(Exception ex) {
            ex.getStackTrace();
        }
